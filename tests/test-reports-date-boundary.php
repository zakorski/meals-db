<?php
/**
 * Regression guard for MAJ-5 — report end-date boundary dropped last-day orders.
 *
 * Before the fix, date-bounded report queries compared
 * `date_created_gmt <= '<end> 00:00:00'`, so any order placed after midnight on
 * the final day of a range was silently excluded ("report through Jan 31" lost
 * all of Jan 31). The fix made every such query half-open [start, end_exclusive):
 * `>= start AND < end_exclusive`.
 *
 * The private normalise_dates() helper that once centralised this bound was
 * removed as dead code (no production caller — each report method inlines its
 * own half-open window), so this file is now a FULL-FILE SOURCE ASSERTION:
 *   T-3  no `<= %s` against date_created_gmt and no legacy `$dates['end']`
 *        survive anywhere in the reports service — the durable guarantee,
 *        independent of any one helper.
 *
