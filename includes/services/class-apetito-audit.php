...
class MealsDB_Apetito_Audit {
    ...
    public static function codes(): array {
        ...
        // is_published = 1 restricts the walk to live products (the spec's
        // "published products"), excluding trashed/draft SKUs that display-sync
        // has already flipped to 0.
        $rows = $wpdb->get_col("SELECT sku FROM `{$table}` WHERE is_published = 1 AND sku REGEXP '^[0-9]{5}$' ORDER BY sku ASC");
        return is_array($rows) ? array_map('strval', $rows) : [];
    }

    /**
     * Audit ONE code (used per cursor tick). Cache-backed fetch; never writes.
     * Carries the product name so the results table can identify the row.
     * @return array{code:string,ok:bool,product?:string,drift?:array,reason?:string}
     */
